...
export default function CategoryJobs() {
  ...
  const filteredJobs = jobs?.filter(job => {
    ...
    // Check if any tag column contains the category name
    const safeArray = (val: any) => Array.isArray(val) ? val : [];
    const tagMatch = [
      ...safeArray(job.education_tags),
      ...safeArray(job.job_type_tags),
      ...safeArray(job.job_area_tags),
      ...safeArray(job.experience_level_tags),
      ...safeArray(job.post_position_tags),
      ...safeArray(job.job_posting_deadline_tags)
    ].some(tag => 
      tag.toString().toLowerCase().includes(categoryName.toLowerCase()) ||
      categoryName.toLowerCase().includes(tag.toString().toLowerCase())
    ) || job.exam_or_post_name?.toLowerCase().includes(categoryName.toLowerCase()) ||
  ...
  const formatQualification = (text: string): string => {
    ...
    const maxTotalChars = 30;
    
    // Truncate if exceeds total character limit
    let processText = text.length > maxTotalChars ? text.substring(0, maxTotalChars) + "..." : text;
    
    const lines: string[] = [];
    
    // Split by comma to preserve natural breaks
    const commaParts = processText.split(',').map(s => s.trim()).filter(s => s.length > 0);
    ...
    for (let i = 0; i < commaParts.length; i++) {
      ...
      // Try to add this part to current line with comma
      const testLine = currentLine 
        ? `${currentLine}, ${part}`
        : part;
      
      // If it fits on the current line, add it
      if (testLine.length <= maxCharsPerLine) {
        ...
        // If the part itself is longer than max, split by words
            ...
            <div className="w-full lg:w-[62%] flex-shrink-0 px-3">
              ...
              <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent p-6 rounded-lg border">
                ...
                <p className="text-muted-foreground mb-4">
                  Latest government job opportunities in {categoryName} sector
                </p>
              ...
              {/* Jobs Table/Cards */}
              <div className="bg-card rounded-lg border shadow-sm jobs-results-section">
                    ...
                    <>
                      ...
                      <div className="md:hidden divide-y">
                        {filteredJobs.map((job) => (
                          <div key={job.job_id} className="py-3 hover:bg-muted/30 transition-colors">
                            <Link to={`/job/${job.page_link || job.job_id}`}>
                              <h3 className="font-semibold text-base text-foreground hover:text-primary transition-colors line-clamp-2 mb-2">
                              ...
                              <TableRow key={job.job_id} className="hover:bg-muted/30 transition-colors">
                                <TableCell className="py-4 px-4">
                                  <div className="space-y-2">
                                    <Link to={`/job/${job.page_link || job.job_id}`}>
                                      <h3 className="font-semibold text-foreground text-sm leading-tight hover:text-primary transition-colors cursor-pointer">
                                        {job.exam_or_post_name}
                                      </h3>
                                    </Link>
