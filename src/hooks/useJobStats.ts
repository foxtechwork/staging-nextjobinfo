...
export function useJobStats() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<JobStats> => {
      ...
      if (isDev) {
        console.warn('⚠️ [useJobStats] Fetching from Supabase (DEV mode)');
    ...
    initialData: () => {
      ...
      const cached = queryClient.getQueryData<JobStats>(['job-stats']);
      if (cached) {
        console.log('✅ Using SSG cached job-stats');
      }
      return cached;
    },
